...
- **An "Acting on tenant" selector in the top bar, for `obs` scope only.** The tenant an OBS user is operating on is one console-wide choice, not a choice repeated on every screen: it is set once, applies to every per-tenant screen, stays put across routes and reloads, and is changeable at any time from the top bar. It is deliberately distinct from the org switcher above — that one changes *who you are* (the active Clerk organization, and so the session's permissions); this one changes *which tenant you act on* while you remain OBS staff. A tenant-scoped user never sees it. The choice is held per signed-in user for the life of the tab, is dropped at sign-out, and resets itself if the stored slug no longer appears in `GET /admin/tenants`. It changes nothing about the wire: **every request still names the tenant explicitly as `?tenant=<slug>`** — OBS acting on a tenant is explicit, never implicit — the selector only centralizes where the UI reads that slug from.
- **Cross-tenant screens are not filtered by it.** All tenants, Platform health and Delivery queue answer questions about the set of tenants and ignore the selection (the All-tenants drawer flows the other way: it *sets* the selection and opens that tenant's Overview). Fan actions already had a tenant filter, so the selection pre-fills it, with "All tenants" still available. Team ignores it — membership reads the active organization from the session, not a tenant slug.
