...
The practical consequence: where the mock and the spec disagree, there is no third authority to appeal to, so the spec decides and says so each time. This happens twice below, both in the capability panel.
...
**Capability panel.** The mock's "What this organization can do" — the tenant's authority, stated plainly, with the things it cannot do shown as explicit negatives. This is the screen's quiet second job: `ADM-02`'s read-only V1 grant and `RPT-02`'s obs-only export are boundaries a tenant admin will otherwise discover as a missing button. Content is derived from the resolved scope.

**The mock's panel overstates a tenant's V1 authority, and the screen corrects it — spec wins over mock, twice.**

- The mock marks "Enable games and attach prize tiers" and "Configure signup fields and opt-ins" as things the organization *can* do. In V1 it cannot. `ADM-02` scopes team users to read-only, `org:tenant_config:manage` is "**Not yet granted to any tenant role in V1**", and admin-surface Rule 3 says it "never appears on a tenant org role set in V1" — the Fields & Opt-ins and Games modules both implement that as an obs-only write. A panel claiming otherwise would be this screen contradicting the two screens it names, and would teach a tenant admin to expect a button that is deliberately absent. Both render for a tenant as **view-only capabilities** — view active games and prize tiers, view signup fields and opt-ins — with the write half stated as a negative pending `ADM-03` **[FUTURE]**. The mock draws the `ADM-03` end state; V1 is not there yet.
- The mock's amber "Finalize a contest — authority undecided (`PRIZE-03`)" is **stale**, and renders as a plain negative. `admin-surface.spec.md` settled it after the mock was drawn: `org:contest:finalize` is "**obs only** (decision, 2026-09)", one of three permissions that must never appear on a tenant role. The design note's own "PRIZE-03 still undecided" caveat is stale for the same reason, and the Games & Prizes module already shipped against the settled answer. An undecided marker on a decided question invites re-litigation on a screen whose whole subject is who may do what.
...
OBS staff who need to act *inside* a tenant org still can, without this screen inventing anything: the `<OrganizationSwitcher>` the admin-surface spec already specifies changes the active organization, and `/team` then shows that org's membership because the session says so. The boundary is Clerk's, the same one, at every moment.
...
`org:sys_memberships:manage` is a **Clerk system permission** — `admin-surface.spec.md`'s naming rule notes system permissions "must not be invented", and this one is not ours to grant or withhold. It is attached to Clerk's default `org:admin` role. None of our custom `org:*` permissions appear in this module; membership is orthogonal to the read/write split, which is the point the admin-surface spec makes when it says the `org:admin`/`org:member` distinction "is not the read/write split".
