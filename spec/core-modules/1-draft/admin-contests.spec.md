...
- **`GET /admin/games`** gains per contest `gameType`, `createdAt`, and `gamesRunCount`; per game row `ranAt` (the contest runs at it now or ran at it before — `false` for a row that is only on the schedule); and at the top level `schedule`: the upcoming games in the candidate window, attached to no contest — what the create and Add games pickers offer, since a tenant with no contests has no games list to borrow from. `numberParticipants` becomes the board count.
- **Upcoming games are never crowded out of the pickers** (corrected 2026-09-23). The candidate window was read as one query, 30 days back to 30 days ahead, oldest first under a single limit of 200: a month with more than 200 played games filled every row, and the pickers, which offer only games that have not started, had nothing to show. The window is now two reads with a limit of 200 each, sorted outwards from now — the games still to come, soonest first, and the games recently played, newest first — so a limit only ever trims the far end. `schedule` is the upcoming half alone. The played half stays in each contest's games as candidates, where it keeps a game that was turned off reachable for a contest stored before `ranAtBetEvents`; the games a contest ran at are resolved by id, as before, and no limit reaches them.
