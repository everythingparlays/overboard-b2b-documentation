...
This is the answer to the obvious objection to a rich editor — "an admin fixing a typo will re-prompt the whole fanbase." They structurally cannot.
...
- **Shared:** component tests for `EntryGateForm` and `EntryGatePreview` — rendering from a config, the validation failure states, and the returning-mode CTA gating; and per-type rendering and validation, `resolveField` merging, copy overrides including blank-falls-back, and the descriptive-edit invariance tests.
...
It **prunes every removed field id from every opt-in's `exportFields`** before writing, and otherwise preserves `exportFields` and `sponsorId` exactly as v1 did — the config screen still never submits either, and a publish from it must not strip a sponsor's DPA scope ([`admin-exports.spec.md`](admin-exports.spec.md)).
