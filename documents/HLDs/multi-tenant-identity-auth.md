...
**`IDN-04` [P0] — Tenant scope is established by middleware and validated against membership, never accepted as a handler parameter.**
Middleware resolves the tenant for a request and validates it against the caller's memberships before any handler runs. Handlers receive tenant and membership as established facts. Any handler accepting an `organizationId` from a query string, body, or path is by definition a defect.

The tenant *selector* is untrusted input — the current deployment proxies `/b2b/*` through Vercel to a publicly-reachable ELB, so neither `Host` nor `X-Forwarded-Host` is more trustworthy than an explicit header. The guarantee does not come from the selector's provenance. It comes from the **membership lookup**, keyed on the Clerk user id in a signed JWT the client cannot forge: a caller who lies about the tenant gets a `403`, because they cannot forge a membership. `IDN-04` is satisfied by making the tenant *useless to forge*, not unforgeable. See the implementation spec for the selector mechanism.
...
- **`IDN-01` holds within an instance, not across the two.** A fan of a phone-primary team *and* an email-primary team has two identities that cannot be linked — no shared profile, no shared prize history, and two rows in any cross-tenant count of people. It is invisible to any fan whose teams sit on one variant, and it is the reason `IDN-09` is P2 rather than P0: the simplification is real, but it buys a genuine and permanent limitation.

  **Revisit trigger:** this is accepted while phone-primary is a niche sale. If phone-primary becomes the majority variant, or if a material number of fans are expected to follow teams across both variants, the trade inverts — at that point a single instance offering both methods is worth the added complexity, and this requirement should be re-opened rather than worked around. Recorded in the PRD under `AUTH-04` so it is visible to whoever sells the variant, not only to whoever builds it.
...
| `IDN-09` | Not built [P2] | One shared instance; no variant concept |
...
**Data model.** Split today's `B2BUser` into identity and membership. A `B2BFan` (Clerk user id, canonical email) and a `B2BFanMembership` (fan × tenant: profile field values per `AUTH-02`, consent records per `IDN-07`, joined-at) matches `AUTH-02`'s per-tenant field configuration and makes `IDN-08` a single-document delete.

**There is nothing to migrate** (decision, 2026-08): dev data is disposable and the production footprint is a handful of POC-era records. Existing fan records are dropped rather than backfilled, and fans re-join on next entry — which also means no fan is grandfathered into a consent they never gave, since consent was never persisted. Individual boards worth keeping are moved by hand, case by case, outside any plan. The alternative shortcut — dropping `unique` on `clerkUserId` for a compound `{clerkUserId, organizationId}` — existed only to avoid a migration, so with no migration to avoid it buys nothing and leaves per-tenant profile data conflated with identity.

**Backend.** Middleware resolves the tenant from an explicit selector header, loads the caller's membership, and injects both — rejecting with `403` when no membership exists. Handlers stop accepting tenant identifiers as input. The consent gate is evaluated server-side and returned with the membership, so the client cannot skip it. `optInConsents` gains a schema.
